...
# 8. 生成 orchestrator Skill（.cursor/skills/orchestrator-{id}/SKILL.md）
SKILLS_DIR="$PROJECT_DIR/.cursor/skills"
echo ""
echo "Generating orchestrator-${PIPELINE_ID}/SKILL.md..."
bun run "$SCRIPT_DIR/generate-orchestrator.ts" "$SELECTED" "$SKILLS_DIR" "$PIPELINE_ID"
...
echo "  .cursor/skills/orchestrator-${PIPELINE_ID}/SKILL.md     → Pipeline orchestrator (Skill)"
echo ""
