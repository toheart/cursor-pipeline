...
function buildStageSection(s: StageDef, vars: Record<string, string>, pid: string): string {
  const lines: string[] = [];
  const label = replaceVars(s.label ?? s.name, vars);
  const pipelineParam = `"pipeline": "${pid}", `;
  ...
  if (s.parallel) {
    ...
    lines.push("```");
    lines.push("");
    lines.push(`在同一条消息中发起多个 Task 工具调用，并行启动（task 描述中包含 \`[pipeline:${pid}]\`）：`);
    lines.push("");
    ...
    lines.push("");
    lines.push("等待所有并行 SubAgent 完成后再进入下一阶段。如果一个失败，另一个的结果仍然保留，只重跑失败的那个。");
    return lines.join("\n");
  }
  ...
  lines.push("");
  lines.push(`委托给 SubAgent \`${s.agent}\`（task 描述中包含 \`[pipeline:${pid}]\`）。`);
  lines.push("Hook 会自动追踪 SubAgent 的 start/stop，建议仍主动标记确保一致性。");
  return lines.join("\n");
}
...
function generateOrchestrator(tpl: Template, pipelineId?: string): string {
  ...
  parts.push(`---
name: ${orchName}
description: 开发流水线编排器（${tpl.name}）。协调 ${stageNames} 全流程。${triggerWords}。支持从中断处恢复。
---

你是 ${projectName} 的开发流水线编排器（pipeline: \`${pid}\`）。你的职责是按顺序调度专家 SubAgent 完成一个完整的功能开发周期。

引用的 Agent：${agentRefs.map(a => `\`${a}\``).join("、")}。

**重要**：调度 SubAgent 时，在 task 描述中包含 \`[pipeline:${pid}]\` 标记，以便 Hook 正确路由事件。`);
...
pipeline-server 运行在 http://127.0.0.1:19090。本 orchestrator 的 pipeline ID 为 \`${pid}\`。
所有 API 调用需在 body 中携带 \`"pipeline": "${pid}"\` 以定位到正确的流水线实例。

### 设置 Change 名称
\`\`\`bash
curl -s -X POST http://127.0.0.1:19090/api/v1/pipeline/change \\
  -H "Content-Type: application/json" \\
  -d '{"pipeline": "${pid}", "name": "<change-name>"}'
\`\`\`
...
curl -s -X POST http://127.0.0.1:19090/api/v1/pipeline/stage \\
  -H "Content-Type: application/json" \\
  -d '{"pipeline": "${pid}", "stage": "<stage-name>", "status": "active|completed|failed|skipped"}'
\`\`\`
...
curl -s -X POST http://127.0.0.1:19090/api/v1/pipeline/gate \\
  -H "Content-Type: application/json" \\
  -d '{"pipeline": "${pid}", "gate": "<gate-name>", "result": "passed|failed"}'
\`\`\`
...
**重要**：SubAgent 阶段的 start/stop 由 Hook 自动追踪（通过 \`[pipeline:${pid}]\` 标记路由），但 Skill 阶段和 Gate 阶段完全依赖主动上报。`);
...
1. 读取 \`.cursor/hooks/state/pipeline-state.json\`，检查是否有未完成的流水线
...
const outFileName = `orchestrator-${pipelineId}.md`;
